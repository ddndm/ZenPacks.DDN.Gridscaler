...
log = logging.getLogger('zen.zenpymodelfs')
...
class ModelFs(DDNModelPlugin):
    """ Models GridScaler Fs """
    relname = 'fsLists'
    modname = 'ZenPacks.DDN.Gridscaler.FsList'

    def prepTask(self, device, log):
        self.device = device
        log.info("%s: preparing for GS filesystems ", device.id)
        cmdinfo = [{
                       'cmd':
                           '/opt/ddn/directmon/gridscaler/scripts/get_gs_metrics.py',
                       'parser': gs.GsFslistParser,
                       'filter': '',
                   }]
    ...
    def parseResults(self, resultList):
        ...
        if errmsgs:
            log.warn('XXXX Gridscaler Fslist collection failed %s',
                     str(errmsgs))
        ...
        log.debug("collected FsLists property: %r" % res)
        d.callback(res)
